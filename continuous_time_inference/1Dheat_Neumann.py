...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, tb, X_f, X_f_val, layers, lb, ub, min_max_f, X_star, u_star):
        ...
        # MAX ABS SCALING
        abs_max_f = tf.cast(tf.reduce_max(tf.abs(min_max_f)), 'float32') # 60994270
        abs_max_u0 = tf.cast(tf.reduce_max(tf.abs(self.u0_tf)), 'float32') # 800
        self.scaled_u0_tf = self.u0_tf / abs_max_u0
        self.scaled_u0_pred = self.u0_pred / abs_max_u0
        self.scaled_u_x_lb_pred = self.u_x_lb_pred / 2e4
        self.scaled_u_x_ub_pred = self.u_x_ub_pred / 2e4
    ...
    def neural_net(self, X, weights, biases):
        ...
        # H = (X - self.lb) / (self.ub - self.lb)                       # SCALING FOR SWISH / SQUARED RELU
    ...
    def train(self, nIter):
        ...
        # OPTIONAL AUTO SCALING AFTER FIRST ITERATIONS
        # for it in range(10):
...
if __name__ == "__main__":
    ...
    data = scipy.io.loadmat('data/heat1D.mat')

    t = data['ts'].flatten()[:, None]
    x = data['xs'].flatten()[:, None]
    Exact = data['usol'].T
    Exact_flux = data['fluxsol'].T
    ...
    fig0.tight_layout()
    fig0.savefig('./figures/1Dheat_Neumann_losses.eps', bbox_inches='tight', pad_inches=0)
    fig0.savefig('./figures/1Dheat_Neumann_losses.pdf', bbox_inches='tight', pad_inches=0)
    ...
    fig0.tight_layout()
    fig0.savefig('./figures/1Dheat_Neumann_error.eps', bbox_inches='tight', pad_inches=0)
    fig0.savefig('./figures/1Dheat_Neumann_error.pdf', bbox_inches='tight', pad_inches=0)
    ...
    ax.set_ylim([Exact.min() - 80, Exact.max() + 80])
    ...
    savefig('./figures/1Dheat_Neumann',crop=False)
